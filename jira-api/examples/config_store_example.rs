use jira_api::{
    AppConfig, Auth, ConfigStore, FileConfigStore, FilterConfig, IssueFilter, JiraConfig, SortOrder,
};
use std::error::Error;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(config) = loaded_app_config {
        ...
        println!(
            "   - 最終更新: {}",
            config.last_updated.format("%Y-%m-%d %H:%M:%S")
        );
    }
...
fn create_my_issues_filter() -> FilterConfig {
    ...
    FilterConfig::new("my_issues".to_string(), "担当課題".to_string(), filter)
