...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("作成日: {}", issue.fields.created.format("%Y-%m-%d %H:%M:%S"));
    println!("更新日: {}", issue.fields.updated.format("%Y-%m-%d %H:%M:%S"));
    ...
    match ChangelogParser::parse_changelog(&issue.id, &issue.key, &changelog_json) {
        Ok(histories) => {
            ...
            for (i, history) in histories.iter().enumerate() {
                ...
                println!("変更日時: {}", history.change_timestamp.format("%Y-%m-%d %H:%M:%S"));
            ...
            // 担当者変更のみ
            let assignee_changes = ChangelogParser::extract_field_changes(
                &histories,
