...
use dotenv::dotenv;
use jira_api::{JiraClient, JiraConfig, SearchParams, TimeBasedFilter};
use std::env;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "    - 更新時刻でフィルタ: {}",
        incremental_filter.filter_by_updated
    );
    ...
    println!(
        "    - 作成時刻でフィルタ: {}",
        detailed_filter.filter_by_created
    );
    println!(
        "    - 更新時刻でフィルタ: {}",
        detailed_filter.filter_by_updated
    );
    println!(
        "    - 除外するIssue数: {} 件",
        detailed_filter.excluded_issue_keys.len()
    );
    ...
    let chunked_filter = TimeBasedFilter::date_range(Utc::now() - Duration::days(3), Utc::now())
    ...
    let test_filters = vec![
        ...
        (
            "無効なフィルター（時間粒度0）",
            TimeBasedFilter::new().granularity_hours(0),
        ),
        (
            "無効なフィルター（両方無効）",
            TimeBasedFilter::new()
