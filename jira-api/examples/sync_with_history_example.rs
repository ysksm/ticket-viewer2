use dotenv::dotenv;
use jira_api::{
    ChangelogParser, DuckDBStore, JiraClient, JiraConfig, PersistenceStore, SyncConfig,
    SyncService, models::SearchParams,
};
use std::env;
use std::error::Error;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !all_histories.is_empty() {
        ...
        println!(
            "{}件のIssueから{}件の履歴レコードを保存しました",
            processed_count, saved_history_count
        );
    } else {
