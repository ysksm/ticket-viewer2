...
fn create_mock_issues(count: usize) -> Vec<Issue> {
    ...
    for i in 1..=count {
        ...
        let priority = Priority {
            ...
            description: Some(format!("{} priority issue", match i % 3 {
        ...
        let fields = IssueFields {
            summary: format!("Hybrid test issue {} - {}", i, match i % 3 {
                0 => "Critical bug",
                1 => "Feature request",
                _ => "General task"
            }),
            ...
            custom_fields: {
                ...
                fields.insert("customfield_10001".to_string(), format!("Hybrid value {}", i).into());
...
async fn run_real_api_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = JiraConfig::from_env()
        .map_err(|_| "Environment variables not set. Please check JIRA_URL, JIRA_USER, JIRA_API_TOKEN")?;
    ...
    if let Some(first_project) = projects.first() {
        println!("   First project: {} ({})", first_project.name, first_project.key);
        ...
        println!("✅ Found {} issues in project {}", 
...
async fn run_mock_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ JSON store filtered results: {} issues", json_filtered.len());
    println!("✅ DuckDB store filtered results: {} issues", duckdb_filtered.len());
    ...
    println!("✅ Deduplication: {} -> {} issues", mock_issues.len() + 5, deduplicated.len());
    ...
    println!("✅ JSON store stats: {} issues, {} projects", 
        json_stats.total_issues, json_stats.issues_by_project.len());
    println!("✅ DuckDB store stats: {} issues, {} projects", 
        duckdb_stats.total_issues, duckdb_stats.issues_by_project.len());
    
    // 統計の整合性確認
    assert_eq!(json_stats.total_issues, duckdb_stats.total_issues);
    assert_eq!(json_stats.issues_by_project.len(), duckdb_stats.issues_by_project.len());
