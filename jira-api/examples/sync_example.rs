...
use dotenv::dotenv;
use jira_api::{JiraClient, JiraConfig, SyncConfig, SyncService, TimeBasedFilter};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "    - 同期間隔: {} 分",
        sync_service.config().interval_minutes
    );
    println!(
        "    - 最大履歴数: {} 回",
        sync_service.config().max_history_count
    );
    println!(
        "    - 時間最適化: {}",
        sync_service.config().enable_time_optimization
    );
    println!(
        "    - 並行処理数: {} 個",
        sync_service.config().concurrent_sync_count
    );
    ...
    match sync_service
        .sync_incremental(&client, &existing_issues)
        .await
    {
        Ok(result) => {
            println!("    [OK] 増分同期完了!");
    ...
    println!(
        "    - 同期間隔: {} 分",
        custom_sync_service.config().interval_minutes
    );
    println!(
        "    - 対象プロジェクト: {:?}",
        custom_sync_service.config().target_projects
    );
    println!(
        "    - 除外フィールド: {:?}",
        custom_sync_service.config().excluded_fields
    );
    ...
    println!(
        "    - 除外フィールド: {:?} (空 = 全フィールド取得)",
        all_fields_sync.config().excluded_fields
    );
    println!(
        "    - 時間最適化: {}",
        all_fields_sync.config().enable_time_optimization
    );

    // テスト同期（実際には大量のデータが取得される）
    println!(
        "    注記: SearchParamsでfields=*allを指定することで全カスタムフィールドが取得されます"
    );
    ...
    let changelog_config = SyncConfig::new()
        ...
        .excluded_fields(vec!["description".to_string(), "comment".to_string()]); // 重いフィールドを除外して基本フィールド + changelogを取得
    ...
    println!(
        "    - 除外フィールド: {:?}",
        changelog_sync.config().excluded_fields
    );
    println!(
        "    - 並行処理数: {} (changelogは重い処理のため)",
        changelog_sync.config().concurrent_sync_count
    );
    ...
    println!(
        "    - 除外フィールド: {:?} (空 = 全フィールド取得)",
        full_sync.config().excluded_fields
    );
    println!(
        "    - 対象プロジェクト: {:?}",
        full_sync.config().target_projects
    );
    println!(
        "    - 同期間隔: {} 分（重い処理のため長め）",
        full_sync.config().interval_minutes
    );
    println!(
        "    - 最大履歴数: {} (メモリ使用量を抑制)",
        full_sync.config().max_history_count
    );
    ...
    // 基本同期（多くのフィールドを除外）
    let basic_config = SyncConfig::new().excluded_fields(vec![
    ...
    // 全フィールド同期（除外なし）
    let all_sync = SyncService::new(SyncConfig::new().excluded_fields(vec![]));

    // 軽量同期（重いフィールドのみ除外）
    let lightweight_sync = SyncService::new(
        SyncConfig::new().excluded_fields(vec!["description".to_string(), "comment".to_string()]),
    );
    ...
    println!(
        "      - 除外フィールド: {:?}",
        basic_sync.config().excluded_fields
    );
    println!("      - 予想データ転送量: 小");
    println!("      - 予想処理時間: 高速");
    ...
    println!(
        "      - 除外フィールド: {:?}",
        lightweight_sync.config().excluded_fields
    );
    println!("      - 予想データ転送量: 中");
    println!("      - 予想処理時間: 中程度");
    ...
    println!(
        "      - 除外フィールド: {:?}",
        all_sync.config().excluded_fields
    );
    println!("      - 予想データ転送量: 大");
    println!("      - 予想処理時間: 低速");
