use dotenv::dotenv;
use jira_api::{DuckDBStore, JiraClient, JiraConfig, PersistenceStore, models::SearchParams};
use std::env;
use std::error::Error;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let jira_token = if env::var("JIRA_API_TOKEN").is_ok() {
        "設定済み"
    } else {
        "未設定"
    };
    let project_key =
        env::var("JIRA_PROJECT_KEY").unwrap_or_else(|_| "未設定（デフォルト: TEST）".to_string());
    ...
    let basic_search_params = SearchParams::new().max_results(3); // fieldsを指定しない = 全フィールド取得
    ...
    let params_json = serde_json::to_string_pretty(&changelog_search_params)
        .unwrap_or_else(|_| "変換エラー".to_string());
    println!("SearchParams JSON:\n{}", params_json);

    match client
        .search_issues(&jql_query, changelog_search_params)
        .await
    {
        Ok(search_result) => {
            println!(
                "✅ changelog検索成功: {}件のIssue",
                search_result.issues.len()
            );

            if search_result.issues.is_empty() {
                println!(
                    "指定されたプロジェクト '{}' にIssueが見つかりませんでした",
                    project_key
                );
