...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 2. 詳細情報付きプロジェクト取得
    println!("\n[2] 詳細情報付きプロジェクト取得");
    let detailed_params = ProjectParams::new().expand(vec![
        "lead".to_string(),
    ...
    match client.get_projects().await {
        Ok(projects) if !projects.is_empty() => {
            ...
            for project in projects.iter().take(3) {
                ...
                // プロジェクトのチケット総数を取得
                let count_params = SearchParams::new().max_results(1).fields(vec![
                    "key".to_string(),
                    "summary".to_string(),
                    "status".to_string(),
                    "reporter".to_string(),
                ...
                match client.search_issues(&jql, count_params).await {
                    Ok(result) => {
                        ...
                        if result.total > 0 {
                            // ステータス分布を取得
                            let status_params = SearchParams::new().max_results(50).fields(vec![
                                "key".to_string(),
                                "summary".to_string(),
                                "status".to_string(),
                                "priority".to_string(),
                            ...
                            match client.search_issues(&jql, status_params).await {
                                Ok(detailed_result) => {
                                    ...
                                    let mut priority_counts: HashMap<String, usize> =
                                        HashMap::new();
                                    let mut type_counts: HashMap<String, usize> = HashMap::new();
                            ...
                            // 最新チケットを表示
                            let latest_params = SearchParams::new().max_results(3).fields(vec![
                                "key".to_string(),
                                "summary".to_string(),
                                "status".to_string(),
                                "reporter".to_string(),
                            ...
                            let latest_jql =
                                format!("project = {} ORDER BY created DESC", project.key);
    ...
    match client.get_projects().await {
        Ok(projects) => {
            ...
            let software_projects: Vec<_> = projects
                .iter()
            ...
            let test_projects: Vec<_> = projects
                .iter()
                .filter(|p| {
                    p.name.to_lowercase().contains("test") || p.name.to_lowercase().contains("demo")
                })
                .collect();
