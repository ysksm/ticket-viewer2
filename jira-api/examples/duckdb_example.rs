...
use dotenv::dotenv;
use jira_api::{
    DateRange, DuckDBStore, IssueFilter, JiraClient, JiraConfig, PersistenceStore, SortOrder,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let search_params = jira_api::SearchParams::new().max_results(100).fields(vec![
        "key".to_string(),
        "summary".to_string(),
        "status".to_string(),
        "priority".to_string(),
...
async fn demonstrate_advanced_filtering(
    ...
    if let Some(project_key) = projects.iter().next() {
        ...
        println!(
            "    複合条件フィルター (プロジェクト: {}, ステータス: Open/In Progress, 過去30日): {} 件",
            project_key,
            complex_results.len()
        );
    ...
    println!(
        "    高優先度Issue (High/Critical): {} 件",
        priority_results.len()
    );
...
/// 高度なソート機能のデモ
async fn demonstrate_advanced_sorting(
    store: &DuckDBStore,
) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n[4] DuckDBの強力なソート機能");
    ...
    for (sort_order, title, description) in sort_demos {
        let filter = IssueFilter::new().sort_order(sort_order).limit(3);
...
/// DuckDBの分析機能デモ
async fn demonstrate_analytics_features(
    store: &DuckDBStore,
) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n[5] DuckDBの分析機能");
...
/// フィルター設定の永続化デモ
async fn demonstrate_filter_persistence(
    ...
    if let Some(config) = loaded_config {
        ...
        println!(
            "      説明: {}",
            config.description.unwrap_or("なし".to_string())
        );
        println!("      使用回数: {} 回", config.usage_count);
        println!(
            "      作成日: {}",
            config.created_at.format("%Y-%m-%d %H:%M")
        );
...
/// DuckDBストアの最適化デモ
async fn demonstrate_optimization(
    store: &mut DuckDBStore,
) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n[7] DuckDBストアの最適化");
    ...
    println!(
        "      • 最適化完了時刻: {}",
        stats_after.last_updated.format("%H:%M:%S")
    );
...
/// 包括的なデモデータの作成
async fn create_comprehensive_demo_data(
    store: &mut DuckDBStore,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=50 {
        ...
        let summary = match issue_type {
            "Bug" => format!(
                ...
                project.0.to_lowercase()
            ),
            "Task" => format!(
                "Implement {} feature for {}",
                priority.to_lowercase(),
                project.1
            ),
            "Story" => format!(
