...
use dotenv::dotenv;
use jira_api::{
    DateRange, IssueFilter, JiraClient, JiraConfig, JsonStore, PersistenceStore, SortOrder,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut store = JsonStore::new(temp_dir.path()).with_compression(true);
    ...
    let search_params = jira_api::SearchParams::new().max_results(50).fields(vec![
        "key".to_string(),
        "summary".to_string(),
        "status".to_string(),
        "priority".to_string(),
...
async fn demonstrate_filtering(store: &JsonStore) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // プロジェクト別フィルタリング
    let projects: std::collections::HashSet<String> = all_issues
        .iter()
        .filter_map(|i| i.fields.project.as_ref().map(|p| p.key.clone()))
        .collect();

    if let Some(project_key) = projects.iter().next() {
        let filter = IssueFilter::new().project_keys(vec![project_key.clone()]);
    ...
    // ステータス別フィルタリング
    let statuses: std::collections::HashSet<String> = all_issues
        .iter()
        .map(|i| i.fields.status.name.clone())
        .collect();

    if let Some(status) = statuses.iter().next() {
        let filter = IssueFilter::new().statuses(vec![status.clone()]);
    ...
    println!(
        "    サマリーに'test'を含むIssue: {} 件",
        text_filtered_issues.len()
    );
...
async fn demonstrate_sorting(store: &JsonStore) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (sort_order, description) in sort_orders {
        let filter = IssueFilter::new().sort_order(sort_order).limit(5);
...
/// データ管理機能のデモ
async fn demonstrate_data_management(
    store: &mut JsonStore,
    ...
    if !issues_to_delete.is_empty() {
        let keys_to_delete: Vec<String> = issues_to_delete.iter().map(|i| i.key.clone()).collect();
    ...
    println!(
        "      プロジェクト数: {} 個",
        final_stats.issues_by_project.len()
    );
    println!(
        "      ステータス種別数: {} 個",
        final_stats.issues_by_status.len()
    );
...
async fn create_demo_data(store: &mut JsonStore) -> Result<(), Box<dyn std::error::Error>> {
    use chrono::Utc;
    use jira_api::{
        Issue, IssueFields, IssueType, Priority, Project, Status, StatusCategory, User,
    };
    ...
    for i in 1..=10 {
        ...
        let fields = IssueFields {
            summary: format!(
                "Demo issue {} - {}",
                i,
                if i % 2 == 0 { "bug fix" } else { "new feature" }
            ),
            description: Some(serde_json::Value::String(format!(
                "This is a demo issue for testing purposes: {}",
                i
            ))),
            status,
