use dotenv::dotenv;
use jira_api::{
    ChangelogParser, HistoryFilter, HistorySortOrder, JiraClient, JiraConfig, JsonStore,
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !stats.field_change_counts.is_empty() {
        ...
        for (field_name, count) in sorted_fields.iter().take(5) {
            // 上位5フィールド
    ...
    if let Some(first_issue) = search_result.issues.first() {
        ...
        let deleted_count = store
            .delete_issue_history(&[first_issue.key.clone()])
            .await?;
