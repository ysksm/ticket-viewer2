use jira_api::{
    JiraConfig, JiraClient, DuckDBStore, PersistenceStore, HistoryFilter,
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let search_params = SearchParams::new()
        .fields(vec!["key".to_string(), "summary".to_string(), "status".to_string()])
    ...
    if !all_histories.is_empty() {
        ...
        // 特定フィールドの変更のみを抽出
        let status_only = ChangelogParser::extract_field_changes(
            &all_histories,
