...
use crate::{Error, IssueHistory, HistoryAuthor};
...
impl ChangelogParser {
    ...
    pub fn parse_changelog(
        ...
        for history_entry in histories_array {
            ...
            let created_str = history_entry
                .get("created")
                .and_then(|c| c.as_str())
                .ok_or_else(|| Error::InvalidData("Missing created timestamp in history".to_string()))?;
            ...
            for item in items {
                let field_name = item
                    .get("field")
                    .and_then(|f| f.as_str())
                    .ok_or_else(|| Error::InvalidData("Missing field name in history item".to_string()))?;
                ...
                history = history.with_field_change(
                    from_value,
                    to_value,
                    from_display,
                    to_display,
    ...
    fn parse_iso_timestamp(timestamp: &str) -> Result<chrono::DateTime<Utc>, chrono::ParseError> {
        ...
        chrono::DateTime::parse_from_rfc3339(&rfc3339_format)
    ...
    /// 変更の統計情報を生成
    pub fn generate_change_summary(histories: &[IssueHistory]) -> std::collections::HashMap<String, usize> {
...
mod tests {
    ...
    fn test_parse_changelog_basic() {
        ...
        let result = ChangelogParser::parse_changelog(
            "10000", 
            "TEST-123", 
            &changelog
        );
    ...
    fn test_parse_changelog_multiple_items() {
        ...
        let assignee_change = histories.iter().find(|h| h.field_name == "assignee").unwrap();
        
        assert_eq!(status_change.from_display_value, Some("Open".to_string()));
        assert_eq!(assignee_change.to_display_value, Some("Jane Doe".to_string()));
    }
    ...
    fn test_extract_field_changes() {
        let histories = vec![
            ...
            IssueHistory::new(
                ...
                Utc::now(), "priority".to_string()
            ),
        ];
        
        let status_changes = ChangelogParser::extract_field_changes(
            &histories,
