...
impl TimeBasedFilter {
    ...
    pub fn to_jql_time_condition(&self) -> Option<String> {
        ...
        if self.exclude_existing && !self.excluded_issue_keys.is_empty() {
            let keys_str = self.excluded_issue_keys
                .iter()
                .map(|k| format!("'{}'", k))
                .collect::<Vec<_>>()
                .join(", ");
        ...
        if conditions.is_empty() {
            ...
            if self.filter_by_created && self.filter_by_updated {
                let created_conditions: Vec<_> = conditions.iter()
                    .filter(|c| c.contains("created"))
                    .cloned()
                    .collect();
                let updated_conditions: Vec<_> = conditions.iter()
                    .filter(|c| c.contains("updated"))
                    .cloned()
                    .collect();
                let other_conditions: Vec<_> = conditions.iter()
                ...
                if !created_conditions.is_empty() || !updated_conditions.is_empty() {
                    let time_condition = format!(
                        "({})",
                        [created_conditions, updated_conditions].concat().join(" OR ")
                    );
...
impl TimeChunk {
    ...
    pub fn to_jql_condition(&self, filter_by_created: bool, filter_by_updated: bool) -> String {
        ...
        if filter_by_created {
            conditions.push(format!("created >= '{}' AND created <= '{}'", start_str, end_str));
        }
        
        if filter_by_updated {
            conditions.push(format!("updated >= '{}' AND updated <= '{}'", start_str, end_str));
        }
...
mod tests {
    ...
    fn test_time_based_filter_validation() {
        ...
        // 時間粒度が0
        let invalid_granularity_filter = TimeBasedFilter::new()
            .granularity_hours(0);
