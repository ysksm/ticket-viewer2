...
use jira_api::{
    Auth, Issue, IssueFields, IssueType, JiraClient, JiraConfig, Priority, Project, SearchParams,
    Status, StatusCategory, User,
};
    ...
    for i in 1..=count {
        ...
        let priority = Priority {
            ...
            icon_url: Some(format!(
                "https://mock-jira.example.com/priority/{}.png",
                i % 3
            )),
...
async fn test_mock_api_connection() {
    ...
    // Given: モッククライアントを作成
    let _client = setup_mock_client().expect("Failed to setup mock client");

    // When: モックプロジェクトデータを作成
    let mock_projects = vec![jira_api::Project {
        id: "10000".to_string(),
        key: "MOCK".to_string(),
        name: "Mock Project".to_string(),
        project_type_key: Some("software".to_string()),
...
async fn test_real_api_search() {
    ...
    let params = SearchParams::new().max_results(5);
...
async fn test_mock_issue_search() {
    ...
    // Then: 検索結果の検証
    println!(
        "✓ Mock search successful: {} total mock issues",
        mock_issues.len()
    );
...
async fn test_real_api_project_search() {
    ...
    let projects = client.get_projects().await.expect("Failed to get projects");
...
async fn test_real_api_auth_error() {
    ...
    // Given: 無効な認証情報で設定を作成
    let base_url = std::env::var("JIRA_URL").expect("JIRA_URL environment variable not set");
    ...
    let config = JiraConfig::new(base_url, invalid_auth).expect("Failed to create config");
