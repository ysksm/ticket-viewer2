...
use jira_api::{
    ...
    Project, SortOrder, Status, StatusCategory, SyncConfig, SyncService, TimeBasedFilter, User,
};
...
async fn test_large_dataset_performance() {
    ...
    let start = Instant::now();
    let json_saved = json_store
        .save_issues(&large_dataset)
        .await
        .expect("Failed to save to JSON store");
    let json_save_time = start.elapsed();

    println!(
        "✓ JSON Store: Saved {} issues in {:?} ({:.2} issues/sec)",
        json_saved,
        json_save_time,
        json_saved as f64 / json_save_time.as_secs_f64()
    );

    let start = Instant::now();
    let json_loaded = json_store
        .load_all_issues()
        .await
    ...
    println!(
        "✓ JSON Store: Loaded {} issues in {:?} ({:.2} issues/sec)",
        json_loaded.len(),
        json_load_time,
        json_loaded.len() as f64 / json_load_time.as_secs_f64()
    );
    ...
    println!(
        "✓ DuckDB Store: Saved {} issues in {:?} ({:.2} issues/sec)",
        duckdb_saved,
        duckdb_save_time,
        duckdb_saved as f64 / duckdb_save_time.as_secs_f64()
    );

    let start = Instant::now();
    let duckdb_loaded = duckdb_store
        .load_all_issues()
        .await
    ...
    println!(
        "✓ DuckDB Store: Loaded {} issues in {:?} ({:.2} issues/sec)",
        duckdb_loaded.len(),
        duckdb_load_time,
        duckdb_loaded.len() as f64 / duckdb_load_time.as_secs_f64()
    );
    ...
    println!(
        "  JSON:   {:.2} ms/issue",
        json_save_time.as_millis() as f64 / json_saved as f64
    );
    println!(
        "  DuckDB: {:.2} ms/issue",
        duckdb_save_time.as_millis() as f64 / duckdb_saved as f64
    );

    println!("Load Performance:");
    println!(
        "  JSON:   {:.2} ms/issue",
        json_load_time.as_millis() as f64 / json_loaded.len() as f64
    );
    println!(
        "  DuckDB: {:.2} ms/issue",
        duckdb_load_time.as_millis() as f64 / duckdb_loaded.len() as f64
    );
...
async fn test_complex_query_performance() {
    ...
    // データ保存
    json_store
        .save_issues(&test_dataset)
        .await
        .expect("Failed to save to JSON");
    duckdb_store
        .save_issues(&test_dataset)
        .await
        .expect("Failed to save to DuckDB");
    ...
    let complex_queries = vec![
        (
            "Project filtering",
            IssueFilter::new().project_keys(vec!["PROJ1".to_string()]),
        ),
        (
            "Status filtering",
            IssueFilter::new().statuses(vec!["Open".to_string(), "In Progress".to_string()]),
        ),
        (
            "Sorted by created",
            IssueFilter::new()
    ...
    for (query_name, filter) in complex_queries {
        ...
        // DuckDBストアのクエリ性能
        let start = Instant::now();
        let duckdb_results = duckdb_store
            .load_issues(&filter)
            .await
...
async fn test_memory_efficiency() {
    ...
    for &size in &data_sizes {
        ...
        println!(
            "  After generation: +{} KB",
            (mem_after_gen.saturating_sub(mem_before)) / 1024
        );
        println!(
            "  After save:       +{} KB",
            (mem_after_save.saturating_sub(mem_after_gen)) / 1024
        );
        println!(
            "  After load:       +{} KB",
            (mem_after_load.saturating_sub(mem_after_save)) / 1024
        );
        println!(
            "  Memory per issue: ~{:.2} KB",
            (mem_after_load.saturating_sub(initial_memory)) as f64 / (size as f64 * 1024.0)
        );
...
async fn test_sync_performance() {
    ...
    for &size in &existing_data_sizes {
        ...
        println!(
            "  Deduplication: {:?} ({} -> {} issues, {:.2} ms/issue)",
            dedup_time,
            size + size / 4,
            deduplicated.len(),
...
async fn test_compression_efficiency() {
    ...
    // 圧縮ありのJSONストア
    let mut json_compressed =
        JsonStore::new(temp_dir.path().join("compressed")).with_compression(true);
    json_compressed
        .initialize()
        .await
        .expect("Failed to initialize compressed store");

    // 圧縮なしのJSONストア
    let mut json_uncompressed =
        JsonStore::new(temp_dir.path().join("uncompressed")).with_compression(false);
    json_uncompressed
        .initialize()
        .await
        .expect("Failed to initialize uncompressed store");
    ...
    json_compressed
        .save_issues(&test_dataset)
        .await
        .expect("Failed to save compressed");
    ...
    json_uncompressed
        .save_issues(&test_dataset)
        .await
        .expect("Failed to save uncompressed");
    let uncompressed_save_time = start.elapsed();

    // ファイルサイズの確認（概算）
    let compressed_stats = json_compressed
        .get_stats()
        .await
        .expect("Failed to get compressed stats");
    let _uncompressed_stats = json_uncompressed
        .get_stats()
        .await
        .expect("Failed to get uncompressed stats");
    ...
    println!(
        "  Compression ratio:      {:.1}%",
        compressed_stats.compression_ratio * 100.0
    );

    // 読み込み速度の比較
    let start = Instant::now();
    let _compressed_loaded = json_compressed
        .load_all_issues()
        .await
        .expect("Failed to load compressed");
    let compressed_load_time = start.elapsed();

    let start = Instant::now();
    let _uncompressed_loaded = json_uncompressed
        .load_all_issues()
        .await
        .expect("Failed to load uncompressed");
    ...
    // 圧縮が効果的であることを確認
    assert!(
        compressed_stats.compression_ratio < 1.0,
...
async fn test_time_filtering_performance() {
    ...
    let time_filters = vec![
        ...
        (
            "Incremental",
            TimeBasedFilter::incremental_since(Utc::now() - Duration::hours(6)),
        ),
        (
            "Custom range",
            TimeBasedFilter::date_range(
    ...
    for (name, filter) in time_filters {
        ...
        assert!(
            chunk_time.as_millis() < 50,
            "Chunk splitting should be fast"
        );
    }
