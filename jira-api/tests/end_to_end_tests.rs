...
use jira_api::{
    DateRange, DuckDBStore, Issue, IssueFields, IssueFilter, IssueType, JsonStore,
    PersistenceStore, Priority, Project, SortOrder, Status, StatusCategory, SyncConfig,
    SyncService, TimeBasedFilter, User,
};
    ...
    for i in 1..=count {
        ...
        let fields = IssueFields {
            ...
            description: Some(serde_json::Value::String(format!(
                "This is test issue {} created for end-to-end testing",
                i
            ))),
            status,
...
async fn test_complete_workflow_end_to_end() {
    ...
    // 3. データ検索とフィルタリングのテスト
    // 基本的な全件取得
    let all_json = json_store
        .load_all_issues()
        .await
        .expect("Failed to load all issues from JSON");
    let all_duckdb = duckdb_store
        .load_all_issues()
        .await
    ...
    let json_filtered = json_store
        .load_issues(&project_filter)
        .await
        .expect("Failed to filter JSON issues");
    let duckdb_filtered = duckdb_store
        .load_issues(&project_filter)
        .await
    ...
    let json_status = json_store
        .load_issues(&status_filter)
        .await
        .expect("Failed to filter by status in JSON");
    let duckdb_status = duckdb_store
        .load_issues(&status_filter)
        .await
    ...
    // 4. 統計情報の検証
    let json_stats = json_store
        .get_stats()
        .await
        .expect("Failed to get JSON stats");
    let duckdb_stats = duckdb_store
        .get_stats()
        .await
    ...
    // 削除後の件数確認
    let json_count = json_store
        .count_issues(&IssueFilter::new())
        .await
        .expect("Failed to count JSON issues");
    let duckdb_count = duckdb_store
        .count_issues(&IssueFilter::new())
        .await
    ...
    json_store
        .optimize()
        .await
        .expect("Failed to optimize JSON store");
    duckdb_store
        .optimize()
        .await
        .expect("Failed to optimize DuckDB store");
...
async fn test_sync_workflow_end_to_end() {
    ...
    assert!(
        sync_service.can_sync().await,
        "Should be able to sync after recovery"
    );
...
async fn test_time_filtering_end_to_end() {
    ...
    assert!(
        incremental.is_valid().is_ok(),
        "Incremental filter should be valid"
    );
    ...
    assert!(
        jql_incremental.is_some(),
        "Should generate JQL for incremental filter"
    );

    println!("✓ JQL generation works correctly");

    // 4. 時間チャンクの分割テスト
    let chunked_filter = TimeBasedFilter::date_range(Utc::now() - Duration::days(2), Utc::now())
        .granularity_hours(6);
    ...
    // 2日間を6時間単位で分割
    assert!(
        chunks.len() >= 8 && chunks.len() <= 9,
        "Should create 8-9 chunks for 2 days with 6h granularity, got {}",
        chunks.len()
    );
    ...
    assert!(
        complex_filter.is_valid().is_ok(),
        "Complex filter should be valid"
    );

    let complex_jql = complex_filter.to_jql_time_condition();
    assert!(
        complex_jql.is_some(),
        "Should generate JQL for complex filter"
    );
...
async fn test_data_consistency_end_to_end() {
    ...
    for (i, filter) in test_filters.iter().enumerate() {
        ...
        if i == 4 {
            println!(
                "⚠️ Skipping Query 4 (time range filtering) - DuckDBStore implementation pending"
            );
            continue;
        }
        ...
        assert_eq!(
            json_results.len(),
            duckdb_results.len(),
            "Query {} results count mismatch",
            i
        );

        // キーの一致を確認（順序は考慮しない）
        let json_keys: std::collections::HashSet<_> =
            json_results.iter().map(|issue| &issue.key).collect();
        let duckdb_keys: std::collections::HashSet<_> =
            duckdb_results.iter().map(|issue| &issue.key).collect();
    ...
    // 削除後の一貫性確認
    let json_final = json_store
        .load_all_issues()
        .await
        .expect("Failed to load final JSON");
    let duckdb_final = duckdb_store
        .load_all_issues()
        .await
        .expect("Failed to load final DuckDB");
...
async fn test_history_management_end_to_end() {
    use jira_api::{HistoryAuthor, HistoryFilter, HistorySortOrder, IssueHistory};
    ...
    // 3. 履歴データの保存
    let duckdb_saved = duckdb_store
        .save_issue_history(&history_records)
        .await
    ...
    for (i, filter) in filters.iter().enumerate() {
        let duckdb_history = duckdb_store
            .load_issue_history(filter)
            .await
            .expect("Failed to load history from DuckDB");
        let json_history = json_store
            .load_issue_history(filter)
            .await
        ...
        assert_eq!(
            duckdb_history.len(),
            json_history.len(),
            "History filter {} results count mismatch",
            i
        );
    ...
    let duckdb_deleted = duckdb_store
        .delete_issue_history(&delete_keys)
        .await
