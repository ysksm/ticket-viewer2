...
/// 5. 並行処理エラー
use jira_api::{
    Auth, DuckDBStore, Error, Issue, IssueFields, IssueFilter, IssueType, JiraClient, JiraConfig,
    JsonStore, PersistenceStore, Priority, Project, SearchParams, Status, StatusCategory,
    SyncConfig, SyncService, TimeBasedFilter, User,
};
...
async fn test_storage_errors() {
    ...
    store1
        .initialize()
        .await
        .expect("Failed to initialize store1");
    store2
        .initialize()
        .await
        .expect("Failed to initialize store2");
    ...
    let (result1, result2) =
        tokio::join!(store1.save_issues(&issues1), store2.save_issues(&issues2));
    ...
    match (result1, result2) {
        ...
        (Err(e1), Err(e2)) => println!(
            "✓ Both concurrent operations failed appropriately: {}, {}",
            e1, e2
        ),
    }
}
...
async fn test_search_errors() {
    ...
    // 1. 無効なプロジェクトキーでの検索
    let invalid_filter = IssueFilter::new().project_keys(vec![
        "".to_string(),
        "INVALID_PROJECT_WITH_VERY_LONG_NAME_THAT_EXCEEDS_LIMITS".to_string(),
    ]);
    ...
    match result {
        Ok(issues) => {
            assert!(
                issues.is_empty(),
                "Should return empty results for invalid project"
            );
    ...
    let invalid_date_filter = IssueFilter::new().created_range(jira_api::DateRange::new(
    ...
    // 3. 極端に大きな制限値
    let extreme_filter = IssueFilter::new().limit(usize::MAX); // 最大値

    let result = json_store.load_issues(&extreme_filter).await;
    match result {
        Ok(issues) => println!(
            "✓ Extreme limit handled gracefully: {} results",
            issues.len()
        ),
    ...
    // 4. 無効な文字を含む検索
    let text_filter = IssueFilter::new().summary_contains("\0\u{FFFF}\u{10FFFF}".to_string()); // ヌル文字と非文字

    let result = json_store.load_issues(&text_filter).await;
    match result {
        Ok(issues) => println!(
            "✓ Invalid character search handled: {} results",
            issues.len()
        ),
...
async fn test_time_filter_errors() {
    ...
    // 2. ゼロの時間粒度
    let zero_granularity = TimeBasedFilter::new().granularity_hours(0);
    ...
    assert!(
        validation.is_err(),
        "Should reject filter with no time fields"
    );
    ...
    assert!(
        chunks.len() > 1000,
        "Should create many chunks for extreme range"
    );

    // 5. 無効なJQL生成
    let filter_with_empty_exclusions =
        TimeBasedFilter::new().excluded_issue_keys(vec!["".to_string(), "   ".to_string()]); // 空白のキー
...
async fn test_sync_errors() {
    ...
    sync_service
        .set_state_for_test(SyncState::Error("Previous error".to_string()))
        .await;
    ...
    assert_eq!(
        deduplicated.len(),
        1,
        "Should deduplicate to single unique issue"
    );
    ...
    // 5. エラーからの回復テスト
    sync_service
        .set_state_for_test(SyncState::Error("Test error for recovery".to_string()))
        .await;
    ...
    for task_id in 0..concurrent_tasks {
        ...
        let handle = tokio::spawn(async move {
            ...
            match store.initialize().await {
                Ok(_) => match store.save_issues(&task_issues).await {
...
async fn test_type_conversion_errors() {
    ...
    match result {
        Ok(issue) => println!(
            "✓ Parsed issue with extreme values: {} (summary length: {})",
...
async fn test_network_simulation_errors() {
    ...
    let result = client
        .search_issues(&extreme_jql, SearchParams::new())
        .await;
